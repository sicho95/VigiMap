export class StreamPlayer {
  ...
  _build() {
    ...
    el.innerHTML = `
      ...
      <canvas style="position:absolute;top:30px;left:0;pointer-events:none;
        width:100%;height:calc(100% - 30px)" id="sp-cv-${this.cam.id}"></canvas>`;
  ...
  play() {
    ...
    // ── HLS m3u8 → <video> natif (pixels accessibles au CV) ───────────────
    if (url.includes('.m3u8') || url.includes('/hls/') || url.includes('googlevideo')) {
    ...
    // ── YouTube embed (CV aveugle mais affichage OK) ───────────────────────
