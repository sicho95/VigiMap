...
const SC = {
  ...
  pinned:'#a371f7', selected:'#58a6ff', match:'#f0883e', unknown:'#484f58',
};
...
function mkIcon(s) {
  ...
  return L.divIcon({
    html: `<svg width="18" height="24" viewBox="0 0 18 24" xmlns="http://www.w3.org/2000/svg">
      <path d="M9 0C4.03 0 0 4.03 0 9c0 6.75 9 15 9 15s9-8.25 9-15C18 4.03 13.97 0 9 0z" fill="${c}"/>
...
export class MapManager {
  constructor(id, onClick) {
    this._id = id; this._onClick = onClick;
  ...
  init() {
    ...
    this._injectStyles();
    this._addLegend();   // ← légende Leaflet bas-gauche
    return this;
  }
  ...
  // ── Légende Leaflet (bas gauche, comme v1) ──────────────────────────────
  _addLegend() {
    const Legend = L.Control.extend({
      ...
      onAdd() {
        ...
        div.innerHTML = [
          ...
          `<span class="ldot" style="background:${c}"></span>${l}`
        ).join('');
        return div;
      }
    });
